...
def check_aws_region() -> Tuple[bool, Optional[str]]:
    """Check if AWS region is configured and is ca-central-1"""
    success, output = run_command(["aws", "configure", "get", "region"])
    if success and output:
        region = output.strip()
        if region == "ca-central-1":
            print_success("AWS region is correctly set to ca-central-1")
            return (True, region)
        else:
            print_error(f"AWS region is set to '{region}' but must be 'ca-central-1'")
            print_info("This solution is only supported in ca-central-1")
            print_info("Change region: aws configure set region ca-central-1")
            return (False, region)
    else:
        print_error("AWS region is not configured")
        print_info("Configure region: aws configure set region ca-central-1")
        return (False, None)
...
def collect_deployment_parameters(account_id: str = None) -> Dict[str, Any]:
    ...
    # AWS Configuration (region is fixed to ca-central-1)
    print_header("AWS Configuration")
    config["aws"] = {
        "region": "ca-central-1",  # Fixed to ca-central-1 as verified in pre-checks
    ...
    print_info("Region is fixed to ca-central-1 (verified in pre-checks)")
    ...
    while True:
        ...
        if is_valid:
            ...
            if check_s3_bucket_exists(bucket_name, "ca-central-1"):
                print_warning(
